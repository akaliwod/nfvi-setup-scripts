...
image_source="ubuntu1404.img"
flavor_name="m1.small"
port_name="management-port-"$vm_name
webserver_ip=$CONFIG_CALAMARI_MGMT
webserver_gw=$CONFIG_MANAGEMENT_GATEWAY
floating_ip=$CONFIG_CALAMARI_FLOATING
...
esc_uuid=`nova list | grep " $vm_name " | awk '{ print $2 }'`
if [ -z $esc_uuid ]; then
...
img_uuid=`glance image-list | grep "\b$image_name\b" | awk '{ print $2 }'`
if [ -z $img_uuid ]; then
